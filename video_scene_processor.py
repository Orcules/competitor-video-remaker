#!/usr/bin/env python3
"""
Video Scene Processor - Competitor Video Remaker
================================================
...
class Config:
    ...
    GOOGLE_SHEET_TAB: str = os.environ.get("GOOGLE_SHEET_TAB", "Input")
    ...
    AWS_BUCKET_NAME: str = os.environ.get("AWS_BUCKET_NAME", "your-s3-bucket")
    ...
    # NOTE: with AdaptiveDetector (PYSCENEDETECT_USE_ADAPTIVE=True) the threshold is an
    # adaptive ratio (~3.0 default; lower = more sensitive). The 20-35 range only applies
    # to the plain ContentDetector scale.
    ...
    def __post_init__(self):
        ...
        # Regionally representative casting/style guidance per target market, so generated
        # visuals match local talent and settings (descriptive localization data, not prescriptive)
        self.CULTURAL_STYLES = {
            'latam': {
                'casting': 'Latin American/Hispanic features, warm skin tones, dark hair',
            ...
            'western_europe': {
                'casting': 'diverse Western European features, mix of skin tones',
            ...
            'eastern_europe': {
                'casting': 'Eastern European/Slavic features, fair to medium skin tones',
            ...
            'east_asia': {
                'casting': 'East Asian features, Chinese/Japanese/Korean appearance',
            ...
            'southeast_asia': {
                'casting': 'Southeast Asian features, warm skin tones',
            ...
            'metap': {
                'casting': 'Middle Eastern, South Asian, or African features as appropriate',
            ...
            'namer': {
                'casting': 'diverse North American features, multicultural mix',
...
class FFmpegProcessor:
    ...
    def check_ffmpeg_installed(cls) -> bool:
        ...
        try:
            ...
            if cls._ffmpeg_available:
                ...
                logger.warning("FFmpeg not found, will use cloud processing via Rendi.dev")
            return cls._ffmpeg_available
        except Exception:
            cls._ffmpeg_available = False
            logger.warning("FFmpeg not installed locally, will use cloud processing via Rendi.dev")
            return False
    ...
    def _detect_scenes_pyscenedetect(
        ...
        try:
            ...
            if video_duration <= 0:
                video_duration = 30.0
                logger.warning(f"Could not determine video duration, using fallback: {video_duration}s")
                ...
                if scene_duration > max_scene_duration:
                    ...
                    for j in range(1, num_splits + 1):
                        ...
                        if new_ts < next_ts - 0.5:  # Don't add if too close to next scene
                            ...
                            logger.info(f"Splitting long scene: added timestamp at {new_ts:.2f}s")
            ...
            for i, ts in enumerate(split_timestamps):
                ...
                if scene_duration >= min_scene_duration or i == 0:  # Always keep first scene
                    ...
                    logger.info(f"Filtering out scene at {ts:.2f}s (duration: {scene_duration:.2f}s < {min_scene_duration}s)")
    ...
    def _detect_scenes_ffmpeg(video_path: str, threshold: float = 0.3) -> List[float]:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning("FFmpeg scene detection failed, using equal intervals")
    ...
    def _extract_frames_cloud(
        ...
        try:
            ...
            logger.warning("Cloud frame extraction requires video URL - using URL-based extraction")
            return []
    ...
    def extract_audio(video_path: str, output_path: str) -> bool:
        ...
        """
        if not FFmpegProcessor.check_ffmpeg_installed():
            logger.warning("FFmpeg not available for local audio extraction")
            return False
...
class GeminiService:
    ...
    def __init__(self, api_key: str, s3_service=None):
        ...
        if not api_key:
            logger.warning("Gemini not available - KIE_API_KEY not set")
            return
        ...
        try:
            ...
            if result.returncode != 0 or not os.path.isfile(out_path):
                ...
                try:
                    ...
                    logger.debug("Temp cleanup failed: %s", _e)
                return (video_path, None)
...
Return valid JSON only."""
    ...
    def _upload_video_to_s3(self, video_path: str) -> Optional[str]:
        ...
        if not self.s3_service:
            logger.warning("S3 service not available for video upload")
            return None
    ...
    def _cleanup_s3_video(self, video_url: str):
        ...
        try:
            ...
            if len(parts) > 1:
                ...
                logger.info("Cleaned up S3 video file")
        except Exception:
    ...
    def _get_cultural_adaptation_instructions(self, target_language: str) -> str:
        ...
        cultural_mapping = {
            ...
            'en': {
                ...
                'casting': 'diverse American population - Caucasian, African American, Hispanic, Asian American',
            ...
            'en-GB': {
                ...
                'casting': 'British population - diverse, including British Asian, British African',
            ...
            'es': {
                ...
                'casting': 'Hispanic/Latino - Mexican, Colombian, Argentine features, warm skin tones, dark hair',
            ...
            'de': {
                'region': 'Western Europe',
                'country': 'Germany',
                'casting': 'German/Central European - fair to light skin, varied hair colors',
            ...
            'fr': {
                'region': 'Western Europe',
                'country': 'France',
                'casting': 'French population - diverse including African French, North African French',
            ...
            'ar': {
                ...
                'casting': 'Arab/Middle Eastern - olive to brown skin tones, dark hair, Middle Eastern features',
            ...
            'he': {
                'region': 'Middle East',
                'country': 'Israel',
                'casting': 'Israeli/Jewish - diverse including Ashkenazi, Sephardi, Mizrahi, Ethiopian',
            ...
            'pt': {
                'region': 'Latin America',
                'country': 'Brazil',
                'casting': 'Brazilian - very diverse, mixed race, African Brazilian, European Brazilian',
            ...
            'it': {
                ...
                'country': 'Italy',
                'casting': 'Italian/Mediterranean - olive skin, dark hair, Southern European features',
            ...
            'ja': {
                'region': 'East Asia',
                'country': 'Japan',
                'casting': 'Japanese - East Asian features, typically black hair',
            ...
            'ko': {
                'region': 'East Asia',
                'country': 'South Korea',
                'casting': 'Korean - East Asian features, typically black hair, K-beauty aesthetic',
            ...
            'zh': {
                'region': 'East Asia',
                'country': 'China',
                'casting': 'Chinese - East Asian features, typically black hair',
            ...
            'ru': {
                ...
                'country': 'Russia',
                'casting': 'Russian/Slavic - fair skin, varied hair colors, Eastern European features',
            ...
            'hi': {
                'region': 'South Asia',
                'country': 'India',
                'casting': 'Indian/South Asian - brown skin tones, dark hair, diverse Indian features',
            ...
            'tr': {
                ...
                'country': 'Turkey',
                'casting': 'Turkish - Mediterranean to Middle Eastern, olive skin, dark hair',
            ...
            'pl': {
                'region': 'Eastern Europe',
                'country': 'Poland',
                'casting': 'Polish/Slavic - fair skin, varied hair colors, Eastern European features',
            ...
            'th': {
                ...
                'country': 'Thailand',
                'casting': 'Thai/Southeast Asian - tan skin, dark hair, Southeast Asian features',
            ...
            'vi': {
                'region': 'Southeast Asia',
                'country': 'Vietnam',
                'casting': 'Vietnamese - East/Southeast Asian features, typically black hair',
        ...
        cultural_info = cultural_mapping.get(lang_code, {
            ...
            'casting': 'diverse population appropriate for the target region',
...
"""
    ...
    def analyze_video_comprehensive(
        ...
        try:
            ...
            if not video_url:
                logger.warning("Could not upload video to S3, falling back to GPT-4o")
                return self._get_empty_analysis()
...
Return valid JSON only."""
            ...
            for attempt in range(max_retries + 1):
                ...
                if result.get("code") is not None and result.get("code") != 200:
                    ...
                    logger.warning(f"Kie.ai Gemini returned code={result.get('code')}: {err_msg}")
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(f"No choices in Gemini response. Retrying in {wait_sec}s...")
                    time.sleep(wait_sec)
                else:
...
1. **REPLACE CHARACTERS (DO NOT USE ORIGINAL!):**
   - Use: {cultural_style.get('casting', 'diverse features appropriate for target region')}
...
REMEMBER: The product_description will be used DIRECTLY for image generation. Make it so detailed that an artist could draw the exact product without ever seeing it!"""
            ...
            if content is None:
                logger.warning("[PRODUCT] OpenAI returned None content")
                return {"has_product": False}
            ...
            if result.get("has_product") and result.get("overall_confidence", 0) < min_confidence:
                logger.info(f"[PRODUCT] Product detected but confidence too low: {result.get('overall_confidence'):.2f} < {min_confidence}")
...
{{
    ...
    "product_integration_plan": {{
        ...
        "lifestyle_scenes": [scene numbers with product in context]
            ...
            content = response.choices[0].message.content
            if content is None:
                logger.warning("[STRUCTURE] Analysis returned None")
                return {"video_structure": "unknown", "scene_plan": []}
...
{{
    ...
    "motion_style_guide": "Description of how motion/animation should feel to match this video's style"
            ...
            content = response.choices[0].message.content
            if content is None:
                logger.warning("[STYLE] Analysis returned None")
                return {}
...
OUTPUT ONLY the corrected, realistic prompt text.""".format(scene_context=scene_context)
            ...
            content = response.choices[0].message.content
            if content is None:
                logger.warning("[PRODUCT] Enhancement returned None, using original prompt")
...
{base_system_prompt}"""
            ...
            content = response.choices[0].message.content
            if content is None:
                logger.warning("OpenAI returned None content for image prompt")
...
- second_prompt: Motion prompt (camera first, then minimal subtle motion, no new elements)"""
            ...
            content = response.choices[0].message.content
            if content is None:
                logger.warning("OpenAI returned None content for motion prompt")
...
{system_prompt}"""
            ...
            content = response.choices[0].message.content
            if content is None:
                logger.warning(f"{row_prefix}OpenAI returned None content")
                return self._empty_video_analysis_result(pyscenedetect_timestamps, video_duration)
...
Respond with ONLY the music description in 2-3 sentences, nothing else. Be specific and creative."""
            ...
            logger.warning(f"Could not generate music description: {e}")
...
{prompt}"""
                ...
                logger.warning(f"No reference image - using nano-banana (text-only)")
...
class RendiService:
    ...
    def _trim_video_local(self, video_url: str, duration: float) -> Optional[str]:
        ...
        if not self._ffmpeg_available():
            return None
        if not self.s3_service:
            logger.warning("Local trim fallback requires S3 service for upload")
            return None
        tmp_dir = tempfile.mkdtemp()
        try:
            ...
            logger.warning(f"Local trim fallback failed: {e}")
            return None
        finally:
            try:
    ...
    def _concatenate_videos_local(self, video_urls: List[str]) -> Optional[str]:
        ...
        if not self._ffmpeg_available() or not video_urls:
            return None
        if not self.s3_service:
            logger.warning("Local concat fallback requires S3 service for upload")
            return None
        tmp_dir = tempfile.mkdtemp()
        try:
            inputs = []
            ...
            logger.warning(f"Local concat fallback failed: {e}")
            return None
        finally:
            try:
    ...
    def _add_audio_to_video_local(self, video_url: str, audio_url: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Local add_audio fallback failed: {e}")
            return None
        finally:
            try:
                ...
                if status == "SUCCESS":
                    ...
                    if scene_info_url:
                        try:
                            ...
                            logger.warning(f"Could not download/parse scene info file: {e}")
    ...
    def get_video_duration_cloud(self, video_url: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Rendi video duration failed ({e}), trying local ffprobe...")
            if self._ffmpeg_available():
                ...
                if dur is not None:
                    ...
                    return dur
            logger.error(f"Error getting video duration: {e}")
        logger.warning("Could not determine video duration, using default 30s")
        return 30.0
    ...
    def get_audio_duration_cloud(self, audio_url: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Rendi audio duration failed ({e}), trying local ffprobe...")
            if self._ffmpeg_available():
    ...
    def trim_video(self, video_url: str, duration: float) -> Optional[str]:
        ...
        try:
            logger.info(f"Trimming video to {duration:.2f}s (with re-encode for clean cuts)...")
            url = f"{self.base_url}/v1/run-ffmpeg-command"
            ffmpeg_command = (
            ...
            logger.warning(f"Rendi trim failed ({e}), trying local FFmpeg...")
    ...
    def slow_motion_video(self, video_url: str, speed_factor: float, target_duration: float = None) -> Optional[str]:
        ...
        try:
            ...
            if speed_factor >= 0.99:
                logger.info(f"Speed factor {speed_factor:.2f} is too close to 1.0, skipping slow motion")
                return video_url
            
            slowdown_percent = (1.0 - speed_factor) * 100
            logger.info(f"Applying slow motion ({slowdown_percent:.0f}% slower, speed={speed_factor:.2f}x)...")
    ...
    def trim_videos_batch(self, video_durations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in video_durations:
            if isinstance(item, dict):
                ...
                if item.get("video_url"):
                    ...
                    logger.warning(f"Skipping image in trim_videos_batch (only videos should be trimmed): {item.get('image_url', '')[:60]}...")
            else:
                # If it's a string, assume it's a video URL
                filtered_video_durations.append(item)
        
        if not filtered_video_durations:
            logger.warning("No video URLs found for trimming (only images were provided)")
            return []
        
        logger.info(f"Trimming {len(filtered_video_durations)} videos in PARALLEL...")
        ...
        def trim_single(item: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                if actual_duration <= 0:
                    ...
                    logger.warning(f"Scene {idx + 1}: Could not get video duration, assuming {actual_duration}s")
                ...
                if target_duration < actual_duration:
                    ...
                    if duration_ratio <= max_slowdown:
                        ...
                        if slowmo_url:
                            ...
                            logger.warning(f"Scene {idx + 1}: Slow motion failed, using original video")
                            final_url = video_url
                ...
                if final_url:
                    ...
                    logger.warning(f"Scene {idx + 1}: Processing failed, using original")
                    return {
                        "video_url": video_url,
        ...
        for v in trimmed_results:
            ...
            if video_url and video_url not in seen_urls:
                ...
                logger.warning(f"Duplicate video URL in trimmed results (index {v.get('index', 0)}): {video_url[:60]}... - removing duplicate")
        
        if len(unique_trimmed_results) < len(trimmed_results):
            logger.warning(f"Removed {len(trimmed_results) - len(unique_trimmed_results)} duplicate videos from trimmed results")
            ...
            for item in video_data:
                if isinstance(item, dict):
                    ...
                    if item.get("video_url"):
                        ...
                        logger.warning(f"Skipping image URL in concatenation (only videos should be concatenated): {item.get('image_url', '')[:60]}...")
                else:
                    # If it's a string, assume it's a video URL
                    filtered_video_data.append(item)
        ...
        try:
            ...
            for url in video_urls:
                if url and url not in seen_urls:
                    ...
                    logger.warning(f"Duplicate video URL detected and removed: {url[:60]}...")
            
            num_videos = len(unique_urls)
            if num_videos != len(video_urls):
                logger.warning(f"Removed {len(video_urls) - num_videos} duplicate videos from concatenation")
            ...
            logger.warning(f"Rendi simple concat failed ({e}), trying local FFmpeg...")
    ...
    def _concatenate_with_transitions(
        ...
        try:
            ...
            logger.warning(f"Rendi xfade concat failed ({e}), trying local FFmpeg...")
    ...
    def add_audio_to_video(self, video_url: str, audio_url: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Rendi add_audio failed ({e}), trying local FFmpeg...")
    ...
    def validate_video_has_audio(self, video_url: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"Could not validate audio track: {e}, assuming video has audio")
...
class ElevenLabsService:
    ...
    def detect_speech_in_audio(self, audio_path: str) -> bool:
        ...
        try:
            if not self.openai_client:
                logger.warning("No OpenAI client for speech detection, assuming speech present")
                return True
            ...
            logger.warning(f"Speech detection failed: {e}, assuming speech present")
    ...
    def text_to_speech_with_timestamps(
        ...
        try:
            ...
            if not characters or not char_starts or not char_ends:
                logger.warning("No alignment data in response, returning audio only")
                return audio_bytes, []
    ...
    def _convert_alignment_to_word_segments(
        ...
        if len(characters) != len(char_starts) or len(characters) != len(char_ends):
            logger.warning("Mismatched alignment arrays")
            return []
    ...
    def get_transcript_from_audio(self, audio_path: str) -> Optional[str]:
        ...
        try:
            if not self.openai_client:
                logger.warning("No OpenAI client for transcription")
                return None
    ...
    def detect_vo_gender(self, audio_path: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            if not self.openai_client:
                logger.warning("No OpenAI client for gender detection")
            ...
            try:
                ...
                if 'm' in gender_response and 'f' not in gender_response:
                    ...
                    if 'male' in gender_response and 'female' not in gender_response:
                        ...
                        logger.warning(f"Could not parse gender from response: {gender_response}")
                        gender = None
                ...
                logger.warning(f"Audio gender detection failed: {e}, trying text-based fallback...")
        ...
        try:
            ...
            if make_public:
                try:
                    ...
                    logger.warning(f"Could not set public ACL: {e}")
        ...
        try:
            ...
            if make_public:
                try:
                    ...
                    logger.warning(f"Could not set public ACL: {acl_err}")
            url = f"https://{self.bucket}.s3.amazonaws.com/{urllib.parse.quote(key)}"
            ...
            try:
                ...
                if status == "completed":
                    ...
                    if download_url:
                        ...
                        logger.warning("ZapCap completed but no download URL")
                        return None
                ...
                logger.warning(f"Error checking ZapCap status: {e}")
                time.sleep(10)
...
class SunoMusicService:
    ...
    def detect_lyrics_in_audio(self, audio_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.warning(f"Could not detect lyrics: {e}")
            return False, ""
    ...
    def _try_upload_cover(self, audio_url: str, style: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Suno upload-cover API error: {response.status_code}")
                return None
            
            result = response.json()
            if result.get("code") != 200:
                logger.warning(f"Suno upload-cover returned error: {result.get('msg')}")
                return None
            
            task_id = result.get("data", {}).get("taskId")
            if not task_id:
                logger.warning("No task ID returned from Suno upload-cover")
                return None
            ...
            logger.warning(f"Suno upload-cover error: {e}")
            return None
    ...
    def _wait_for_music(self, task_id: str, timeout: int = 600) -> Optional[str]:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                result = response.json()
                if result.get("code") != 200:
                    logger.warning(f"Suno status check error: {result.get('msg')}")
                    time.sleep(15)
                    continue
                ...
                logger.warning(f"Error checking Suno status: {e}")
                time.sleep(15)
    ...
    def upload_video_from_url(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"ACL not supported, using direct URL (bucket is likely public)")
    ...
    def upload_product_reference(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[PRODUCT] ACL not supported, using direct URL")
...
class VideoSceneProcessor:
    ...
    def __init__(self):
        ...
        if config.ENABLE_GEMINI_VIDEO_ANALYSIS and config.KIE_API_KEY:
            ...
            if config.ENABLE_GEMINI_VIDEO_ANALYSIS:
                logger.warning("Gemini video analysis enabled but KIE_API_KEY not set")
        ...
        if config.ZAPCAP_API_KEY:
            ...
            logger.info("ZapCap service not available (no API key)")
    ...
    def process_all_videos(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Manual instructions column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"CTA button column not found, proceeding without")
            cta_button_col = None
        ...
        try:
            ...
            logger.info(f"CTA text column not found, proceeding without")
            cta_text_col = None
        ...
        try:
            ...
            logger.info(f"CTA Duration column not found, defaulting to 'At the End'")
        ...
        try:
            ...
            logger.info(f"Add subtitles column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"Opening Text? column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"Article column not found, proceeding without")
            article_col = None
        ...
        try:
            ...
            logger.info(f"Vertical column not found, proceeding without")
            vertical_col = None
        ...
        try:
            ...
            logger.info(f"Language column not found, proceeding without")
            language_col = None
        ...
        try:
            ...
            logger.info(f"Manual VO text column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"Manual music link column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"Free text column not found, proceeding without")
            free_text_col = None
        ...
        try:
            ...
            logger.info(f"Time column not found, proceeding without")
            time_col = None
        ...
        try:
            ...
            logger.info(f"Voice id column not found, using default voice")
            voice_id_col = None
        ...
        try:
            ...
            logger.info(f"Animation model column not found, using default (runway)")
        ...
        try:
            ...
            logger.info(f"Title column not found, proceeding without")
            title_col = None
        ...
        try:
            ...
            logger.info(f"1stP column not found, proceeding without")
            first_para_col = None
        ...
        try:
            ...
            logger.info(f"Rest of Content column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"Article related to Video column not found, defaulting to 'Yes' behavior")
        ...
        try:
            ...
            logger.info(f"Product image (optional) column not found, proceeding without")
        ...
        try:
            ...
            logger.info(f"Final Video column not found, cannot skip completed rows")
        ...
        def process_row(row_idx: int, row_data: List[str]) -> Optional[Dict[str, Any]]:
            ...
            if final_video_col is not None and final_video_col < len(row_data):
                if row_data[final_video_col].strip():
                    logger.info(f"Row {row_num}: already has Final Video — skipping")
                    return None
    ...
    def process_single_video(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not get duration via OpenCV: {e}")
            ...
            try:
                ...
                if not audio_extracted:
                    ...
                    if original_audio_url:
                        try:
                            ...
                            logger.debug("Audio extraction failed: %s", _e)
                ...
                if audio_extracted and os.path.exists(audio_path):
                    ...
                    if original_transcript:
                        ...
                        logger.info(f"[Row {row_num}] No speech detected in video")
                else:
                    logger.warning(f"[Row {row_num}] Could not extract audio for transcription")
                    
            except Exception as audio_err:
                logger.warning(f"[Row {row_num}] Audio extraction error: {audio_err}")
            ...
            if self.gemini_service and self.gemini_service.initialized:
                ...
                try:
                    ...
                    if gemini_analysis and gemini_analysis.get("scenes"):
                        ...
                        if recommended_timestamps and product.get("detected"):
                            ...
                            for i, timestamp in enumerate(recommended_timestamps[:5]):  # Max 5 frames
                                try:
                                    ...
                                    logger.warning(f"Could not extract frame at {timestamp}: {frame_err}")
                        ...
                        logger.warning(f"[Row {row_num}] Gemini analysis returned empty results")
                        
                except Exception as gemini_error:
                    logger.warning(f"[Row {row_num}] Gemini analysis failed: {gemini_error}")
                    gemini_analysis = None
            else:
                logger.info(f"[Row {row_num}] Gemini not available, using GPT-4o for analysis")
                ...
                if not product_reference_url:
                    ...
                    if product_frame_urls:
                        ...
                        if clean_product_url:
                            ...
                            try:
                                ...
                                logger.warning(f"Could not save clean product to S3: {upload_err}")
                        else:
                            product_reference_url = product_frame_urls[0]
                            logger.warning(f"Clean product generation failed, using raw frame: {product_reference_url[:60]}...")
                    ...
                    if product_reference_url:
                        try:
                            ...
                            logger.warning(f"Could not write product image to sheet: {e}")
                ...
                try:
                        ...
                        if ref_frame_paths:
                            ...
                            for i, frame_path in enumerate(ref_frame_paths[:3]):
                                try:
                                    ...
                                    logger.warning(f"Could not upload frame {i+1}: {frame_err}")
                            ...
                            if ref_frame_urls:
                                ...
                                if clean_product_url:
                                    try:
                                        ...
                                        product_reference_url = clean_product_url
                                        logger.warning(f"Could not save to S3, using original: {upload_err}")
                                else:
                                    product_reference_url = ref_frame_urls[0]
                                    logger.warning(f"Clean product generation failed, using raw frame")
                            else:
                                logger.warning(f"[Row {row_num}] Failed to upload reference frames")
                        else:
                            logger.warning(f"[Row {row_num}] No valid reference frames found")
                    ...
                    if product_info.get("has_product"):
                        ...
                        try:
                            ...
                            logger.warning(f"[Row {row_num}] Failed to write product info to sheet: {sheet_error}")
                    else:
                        logger.info(f"[Row {row_num}] No product detected, continuing with standard flow")
            ...
            if gemini_analysis and gemini_analysis.get("style"):
                ...
                try:
                    ...
                    if video_style and not video_style.get("error"):
                        ...
                        logger.warning(f"[Row {row_num}] Style analysis failed, using defaults")
                        
                except Exception as style_error:
                    logger.warning(f"[Row {row_num}] Style analysis error: {style_error}")
                    video_style = {}
            ...
            if gemini_analysis and gemini_analysis.get("scenes"):
                ...
                try:
                    ...
                    if video_structure.get("video_structure") != "unknown":
                        ...
                        logger.info(f"[Row {row_num}] Could not determine video structure")
                ...
                if vo_word_count > 0:
                    ...
                    if current_total_duration > 0 and abs(estimated_vo_duration - current_total_duration) > 1.0:
                        ...
                        if 0.7 <= scale_factor <= 1.5:
                            ...
                            logger.info(f"Scale factor {scale_factor:.2f}x out of range (0.7-1.5), keeping original durations")
                            ...
                            if should_remove_text:
                                ...
                                if original_has_text and not original_has_vo:
                                    ...
                                    if scene_text and scene_text.lower().strip() not in ["none", "no text", "no", "n/a", "na", ""]:
                                        ...
                                        logger.warning(f"[Scene {scene_num}] Skipping invalid text (contains 'none' or empty): '{scene_text}'")
...
- The product placement and usage should be logical based on the reference image and the scene context"""
                            ...
                            if product_info.get("has_product"):
                                ...
                                if product_visible_fallback:
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"[Scene {scene_num}] Failed to enhance prompt: {e}")
                            ...
                            if motion_prompt:
                                ...
                                if product_info.get("has_product"):
                                    try:
                                        ...
                                        logger.warning(f"[Scene {scene_num}] Motion enhancement failed: {me}")
                                ...
                                if video_url:
                                    ...
                                    try:
                                        ...
                                        logger.warning(f"[Scene {scene_num}] S3 upload failed, using temp URL: {upload_err}")
            ...
            def process_audio_pipeline():
                ...
                if not audio_extracted or not os.path.exists(audio_path):
                    logger.warning("[AUDIO] Could not extract audio")
                    return
                ...
                if original_has_vo:
                    ...
                    if update_success:
                        ...
                        try:
                            ...
                            logger.warning(f"[Row {row_num}] Failed to re-read Voice ID: {e}")
                    else:
                        logger.warning(f"[Row {row_num}] Failed to write gender to sheet")
                else:
                ...
                if original_has_vo:
                    ...
                    if clean_vocals_path:
                        ...
                        logger.warning("[AUDIO] Stem separation failed, using original audio")
                    ...
                    if manual_music_link:
                        ...
                        if music_url:
                            ...
                            logger.warning("[AUDIO] Could not generate cover music, using original")
            ...
            for scene in scene_data:
                ...
                if scene_num in scene_videos_dict:
                    logger.warning(f"Scene {scene_num}: Already in list, skipping duplicate")
                    continue
                ...
                if scene_num in scene_results:
                    ...
                    if scene_result.get("video_url"):
                        ...
                        logger.warning(f"Scene {scene_num}: Only image available, no video/animation - skipping from concatenation")
                        missing_videos.append(scene_num)
                    else:
                        missing_videos.append(scene_num)
                        logger.warning(f"Scene {scene_num}: video missing or failed")
                else:
                    missing_videos.append(scene_num)
                    logger.warning(f"Scene {scene_num}: video missing or failed")
            ...
            if missing_videos:
                logger.warning(f"{len(missing_videos)} scenes missing videos: {missing_videos}")
            ...
            if scene_videos_with_durations:
                ...
                logger.info(f"Trimming {len(scene_videos_with_durations)} videos to their original scene durations...")
                ...
                for i, video_item in enumerate(trimmed_videos):
                    ...
                    if video_url and "storage.rendi.dev" in video_url:
                        ...
                        if uploaded_url:
                            ...
                            logger.warning(f"Failed to upload Rendi video to S3, using original URL")
                            ...
                            if opening_image_url:
                                ...
                                if opening_processed_url:
                                    ...
                                    if video_with_opening:
                                        ...
                                        logger.warning("Failed to overlay opening text on video")
                                else:
                                    logger.warning("Failed to process opening text image")
                            else:
                                logger.warning("Failed to generate opening text image")
                        else:
                            logger.warning("No opening text available (empty and couldn't generate)")
                    except Exception as e:
                        ...
                        if cta_image_url:
                            ...
                            if cta_processed_url:
                                ...
                                if video_with_cta:
                                    ...
                                    logger.warning("Failed to overlay CTA on video, continuing without CTA")
                            else:
                                logger.warning("Failed to process CTA button image, continuing without CTA")
                        else:
                            logger.warning("Failed to generate CTA button, continuing without CTA")
                    except Exception as e:
                        ...
                        if target_duration:
                            if target_duration > total_video_duration:
                                ...
                                if duration_ratio <= max_slowdown:
                                    ...
                                    logger.info(f"Video ({total_video_duration:.2f}s) is shorter than VO ({target_duration:.2f}s)")
                                    ...
                                    logger.info(f"Video ({total_video_duration:.2f}s) is too short for slow motion ({duration_ratio:.2f}x needed > {max_slowdown:.1f}x max)")
                                ...
                                if excess_ratio > 0.1:  # More than 10% longer
                                    ...
                                    logger.info(f"Video is {excess_ratio*100:.1f}% longer than VO, trimming by {trim_amount:.2f}s...")
                                    ...
                                    if trimmed_last_scene:
                                        ...
                                        logger.warning("Failed to trim video, continuing with current duration")
                                else:
                ...
                for video_item in trimmed_videos:
                    ...
                    if video_url and video_url not in seen_video_urls:
                        ...
                        logger.warning(f"Duplicate video URL detected before concatenation: {video_url[:60]}... - removing duplicate")
                
                if len(unique_trimmed_videos) < len(trimmed_videos):
                    logger.warning(f"Removed {len(trimmed_videos) - len(unique_trimmed_videos)} duplicate videos before concatenation")
                ...
                if combined_video_url:
                    ...
                    if new_voice_url:
                        try:
                            ...
                            if vo_duration > 0:
                                ...
                                if combined_duration > 0 and combined_duration < vo_duration:
                                    ...
                                    if duration_ratio <= max_slowdown:
                                        ...
                                        logger.info(f"Combined video ({combined_duration:.2f}s) is shorter than VO ({vo_duration:.2f}s)")
                                        ...
                                        if slowmo_combined:
                                            ...
                                            logger.warning("Slow motion failed, VO may extend past video end")
                                    else:
                                        # Too much slowdown needed, keep as-is
                                        logger.info(f"Combined video ({combined_duration:.2f}s) is too short for slow motion ({duration_ratio:.2f}x needed > {max_slowdown:.1f}x max)")
                            ...
                            logger.warning(f"Could not verify video duration vs VO: {e}")
            ...
            if combined_video_url and final_audio_url:
                ...
                for audio_attempt in range(MAX_AUDIO_RETRIES):
                    ...
                    if final_video_with_voice:
                        ...
                        if has_audio:
                            ...
                            logger.warning(f"Video has no audio track (attempt {audio_attempt + 1}/{MAX_AUDIO_RETRIES})")
                            final_video_with_voice = None
                    else:
                        logger.warning(f"Audio combination failed (attempt {audio_attempt + 1}/{MAX_AUDIO_RETRIES})")
                ...
                if not final_video_with_voice and combined_video_url:
                    logger.warning("All audio combination attempts failed, trying fallback...")
                        ...
                        for music_attempt in range(MAX_AUDIO_RETRIES):
                            if base_video:
                                ...
                                if final_video_with_music:
                                    ...
                                    logger.warning(f"Music overlay failed (attempt {music_attempt + 1}/{MAX_AUDIO_RETRIES})")
            ...
            if add_subtitles and source_for_subtitles:
                ...
                if not has_speech:
                    ...
                    if subtitled_video_url:
                        ...
                        logger.warning("Could not add subtitles with ZapCap")
                else:
                    logger.warning("ZapCap service not available (no API key)")
                ...
                if image_url:
                    ...
                    if second_prompt:
                        ...
                        if video_url:
                            ...
                            logger.warning(f"Scene {scene_num}: {animation_model.upper()} video generation failed")
                else:
                    logger.warning(f"Scene {scene_num}: Nano Banana image generation failed")
            else:
                logger.warning(f"Scene {scene_num}: No prompt generated by OpenAI")
    ...
    def _process_cta_button(
        ...
        try:
            ...
            if not remove_green_background(original_path, final_path):
                logger.warning("Failed to remove background, using original")
    ...
    def _update_sheet_cell(
        ...
        with self._sheets_lock:  # Brief lock only around the API call (gspread thread-safety)
            try:
                ...
                logger.warning(f"Column '{column}' not found, skipping update")
                return False
...
def main():
    ...
    logger.info("="*60)
    logger.info("Competitor Video Remaker")
    logger.info("="*60)
