...
| `RENDI_API_KEY` | Rendi.dev video assembly (concat, audio merge) |
| `ELEVEN_LABS_API_KEY` | Voiceover generation |
| `ZAPCAP_API_KEY` | ZapCap subtitle generation |
| `ZAPCAP_TEMPLATE_ID` | ZapCap subtitle template to apply |
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | S3 access |
| `AWS_BUCKET_NAME` / `AWS_REGION` | S3 destination |
| `SERVICE_ACCOUNT_FILE` | Path to your Google service account JSON |
| `GOOGLE_SHEET_ID` | ID of the Google Sheet that drives the job queue |
| `GOOGLE_SHEET_TAB` | Worksheet tab with job rows (defaults to `Input`) |
| `GOOGLE_API_KEY` | Gemini video analysis (optional) |
| `GCS_CREDENTIALS_FILE` | Path to the GCS service-account JSON |
| `GCS_BUCKET_NAME` / `GCS_FOLDER_NAME` | GCS bucket + folder for intermediate assets |
| `GCS_VIDEO_BUCKET_NAME` | GCS bucket for final rendered videos |
